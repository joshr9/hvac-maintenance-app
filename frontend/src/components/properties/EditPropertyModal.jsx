// components/properties/EditPropertyModal.jsx
import React, { useState, useEffect } from 'react';
import { X, Building, Save, MapPin } from 'lucide-react';
    ...
    try {
      const response = await fetch(`/api/properties/${property.id}`, {
        ...
        body: JSON.stringify({
          ...
          name: propertyData.name.trim() || propertyData.address.trim()
        }),
      });
      ...
      <div className="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full mx-4">
        ...
        <div className="mt-4 p-3 bg-gray-50 rounded-lg">
          <p className="text-sm text-gray-600">
            ...
            <strong> Suites:</strong> {property.suites?.length || 0} • 
            <strong> HVAC Units:</strong> {property.suites?.reduce((sum, suite) => sum + (suite.hvacUnits?.length || 0), 0) || 0}
          </p>
        </div>
      </div>
    </div>
